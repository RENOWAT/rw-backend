...
public class PdfTicketBuilder {
    ...
    public ResponseEntity<byte[]> generateInvoice(Invoice invoice, Subscription subscription) {
        ...
        pdf.paragraphEmphasized("Detalle de la factura")
                .paragraphEmphasized(invoice.getInvoiceStartDate().format(DateTimeFormatter.ofPattern("yyyy-MM-dd  -  ")) +
                        invoice.getInvoiceEndDate().format(DateTimeFormatter.ofPattern("yyyy-MM-dd")))
                .paragraphEmphasized("Potencia facturada: " + fixedTerm+ " €")
                .paragraphEmphasized("Energia facturada valle: " + invoice.getC1() + " kWh x "
                        + subscription.getPlan().getP1() + " €/kWh  - Total periodo valle " + energyCostC1 + " €")
                .paragraphEmphasized("Energia facturada llano: " + invoice.getC2() + " kWh x "
                        + subscription.getPlan().getP2() + " €/kWh  - Total periodo llano " + energyCostC2 + " €")
                .paragraphEmphasized("Energia facturada punta: " + invoice.getC3() + " kWh x "
                        + subscription.getPlan().getP3() + " €/kWh  - Total periodo punta " + energyCostC3 + " €")
                .paragraphEmphasized("Total energía: " + energyCost + " €")
                .paragraphEmphasized("Total antes de impuestos: " + totalCostBeforeTax + " €")
                .paragraphEmphasized("IVA: " + totalCostBeforeTax
                        .multiply(BigDecimal.valueOf(0.05)).setScale(3, RoundingMode.HALF_UP) + " €")
        ...
        table.tableCells(invoice.getCurrency(), "" + invoice.getAmount().setScale(2, RoundingMode.HALF_UP) + "€");
